...
class PermissionResource(DatabaseResource):
    ...
    def collection(cls, query_arg, meta, user, **kwargs):
        ...
        if only_entity_options:
            return cls.build_result_set([Permission()], user, **kwargs)
    ...
    def update(self, payload, **kwargs):
        ...
        role_ids_create = [
            role_id for role_id in role_ids if role_id not in roles_mapping
        ]
        role_ids_delete = [
            role_id for role_id in roles_mapping.keys() if role_id not in role_ids
        ]
        if role_ids_create:
