...
class SparkApplicationChild:
    @classmethod
    def build_api(cls) -> BaseAPI:
        return API.build()

    @classmethod
    async def get_application_id(cls, **kwargs) -> str:
        application_id = None
        parent_model = kwargs.get('parent_model')
        if parent_model and isinstance(parent_model, Application):
            application_id = parent_model.calculated_id()
        else:
            models = await cls.build_api().applications()
    ...
    @classmethod
    def application_calculated_id_from_query(cls, query: Dict = None) -> str:
        if not query:
            return None
